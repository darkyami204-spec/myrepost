...
function Artists({ navigate }) {
  ...
  // Group artists by genre for display
  const artistsByGenre = {
    'Dutch Legends': ['martin-garrix-bio', 'armin-bio', 'afrojack-bio', 'tiesto-bio', 'r3hab-bio'],
    'Techno Titans': ['amelie-lens-bio', 'charlotte-bio', 'nina-bio'],
    'International Superstars': ['zedd-bio', 'alesso-bio', 'kygo-bio', 'david-guetta-bio', 'calvin-harris-bio', 'chainsmokers-bio']
  };
    ...
    <div className="min-h-screen bg-white text-black font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold mb-6">Artists Directory</h1>
          <p className="text-gray-600 text-lg max-w-3xl mx-auto">
            Explore the world's most influential electronic music artists, from legendary pioneers to modern chart-toppers.
          </p>
        </div>

        {/* Artists Grid by Genre */}
        {Object.entries(artistsByGenre).map(([genre, artistKeys]) => (
              ...
              {artistKeys.map((key) => {
                ...
                return (
                  <div 
                    ...
                    className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition duration-300 cursor-pointer active:scale-[0.98]"
                  >
                    <img 
                      src={artist.img} 
                      alt={artist.title}
                    ...
                    />
                    <div className="p-6">
                      <h3 className="font-bold text-xl mb-3 hover:text-purple-600">{artist.title.split(':')[0]}</h3>
                      <p className="text-gray-600 text-sm mb-4 line-clamp-2">{artist.summary}</p>
                      <div className="text-xs text-gray-500">{artist.author.split('•')[1]?.trim()}</div>
                    </div>
                  </div>
                );
              })}
